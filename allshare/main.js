...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const section = document.querySelector(".section-img-cta");
  if (!section) return;

  const blocks = section.querySelectorAll(".metric-column > *");

  // ⚠️ Définis ici ton ordre visuel fixe
  const displayOrder = [
    blocks[1], // graphique (milieu haut)
    blocks[0], // Contrôle de gestion sociale
    blocks[4], // Finances (bas gauche)
    blocks[2], // Réserver une Démo (haut droite)
    blocks[3], // RH
    blocks[5], // photo des personnes
    blocks[6], // mini cards
    blocks[7], // taux d’âge
  ];

  // État initial invisible, flou et décalé
  gsap.set(displayOrder, {
    opacity: 0,
    y: 40,
    filter: "blur(10px)"
  });

  // Animation déclenchée au scroll dans la vue
  ScrollTrigger.create({
    trigger: section,
    start: "top 80%",
    once: true,
    onEnter: () => {
      gsap.to(displayOrder, {
        opacity: 1,
        y: 0,
        filter: "blur(0px)",
        duration: 0.6,
